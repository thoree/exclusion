...
It could be worthwhile to look at other implementations.
The most relevant I know of, in terms of requiring somewhat simillar input, is `relMix`
```{r, echo = TRUE, eval = FALSE}
library(relMix, quietly = TRUE)
relMixGUI()
```

![](relMixGUI.png)

Some pedigrees (`Paternity`, `unrelated`) are hard coded. Otherwise,
a file is taken as input to describe the pedigree in relMix.
...
```{r, echo = TRUE, eval = TRUE}
claim = nuclearPed(1, father = "4", childre = "5")
true = halfSibPed(1)
f = 1/16
